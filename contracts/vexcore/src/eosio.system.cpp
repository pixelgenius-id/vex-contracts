...
namespace eosiosystem {
   ...
   :native(s,code,ds),
    ...
    _vexlimits(get_self(), get_self().value),
    ...
    _rexmaturity(get_self(), get_self().value)
   {
      ...
      // Only read global4 in new format after migration; old Vexanium global4 has different layout (72 bytes vs 24 bytes)
      if( _vexlimits.exists() ) {
         _gstate4 = _global4.exists() ? _global4.get() : get_default_inflation_parameters();
      } else {
         _gstate4 = get_default_inflation_parameters();
      }
      _gvexlimits  = _vexlimits.exists() ? _vexlimits.get() : vex_limits_state{};
   }
   ...
   system_contract::~system_contract() {
      ...
      // Only write global4 in new format after migration — preserve old Vexanium format until migrate() runs
      if( _vexlimits.exists() ) {
         _global4.set( _gstate4, get_self() );
         _vexlimits.set( _gvexlimits, get_self() );
      }
   }

   // Vexanium: one-time migration of old global4 (daily limits) -> vexlimits
   void system_contract::migrate() {
      require_auth( get_self() );

      check( !_vexlimits.exists(), "migration already done" );

      const auto core_sym = core_symbol();
      vex_limits_state vls;

      // Old Vexanium global4 = 4 assets (16 bytes each) + 1 time_point (8 bytes) = 72 bytes
      // New Antelope global4 = double (8) + int64 (8) + int64 (8) = 24 bytes
      // Use raw db size check to detect if old data is still intact (not yet overwritten by onblock)
      static constexpr uint32_t LEGACY_GLOBAL4_BYTES = 72;
      int raw_itr = eosio::internal_use_do_not_use::db_find_i64(
         get_self().value, get_self().value,
         "global4"_n.value, "global4"_n.value
      );
      bool has_legacy = (raw_itr >= 0) &&
         (eosio::internal_use_do_not_use::db_get_i64(raw_itr, nullptr, 0) >= (int32_t)LEGACY_GLOBAL4_BYTES);

      if( has_legacy ) {
         // Old 72-byte format still intact — migrate actual on-chain values
         global_state4_legacy_singleton legacy_global4( get_self(), get_self().value );
         auto legacy = legacy_global4.get();
         vls.daily_withdraw_limit    = legacy.daily_withdraw_limit;
         vls.daily_undelegate_limit  = legacy.daily_undelegate_limit;
         vls.total_withdrawn_today   = legacy.total_withdrawn_today;
         vls.total_undelegated_today = legacy.total_undelegated_today;
         vls.last_reset_time         = legacy.last_reset_time;
      } else {
         // Old data unavailable (no global4 row, or already overwritten with new 24-byte format)
         // Initialize with mainnet defaults: 200000.0000 VEX each
         vls.daily_withdraw_limit    = asset( 2000000000LL, core_sym );
         vls.daily_undelegate_limit  = asset( 2000000000LL, core_sym );
         vls.total_withdrawn_today   = asset( 0, core_sym );
         vls.total_undelegated_today = asset( 0, core_sym );
         vls.last_reset_time         = eosio::current_time_point();
      }

      _vexlimits.set( vls, get_self() );
      _gvexlimits = vls;

      // Re-initialize global4 with proper Antelope tokenomics defaults
      _gstate4 = get_default_inflation_parameters();
      _global4.set( _gstate4, get_self() );
   }

   // Vexanium: set daily withdrawal limit from REX fund
   void system_contract::setrexlimit( const asset& new_limit ) {
      require_auth( get_self() );
      check( new_limit.symbol == core_symbol(), "limit must be in core token" );
      check( new_limit.amount > 0, "limit must be positive" );
      check( _vexlimits.exists(), "run migrate action first" );
      _gvexlimits.daily_withdraw_limit = new_limit;
   }

   // Vexanium: set daily undelegate bandwidth limit
   void system_contract::setundlimit( const asset& new_limit ) {
      require_auth( get_self() );
      check( new_limit.symbol == core_symbol(), "limit must be in core token" );
      check( new_limit.amount > 0, "limit must be positive" );
      check( _vexlimits.exists(), "run migrate action first" );
      _gvexlimits.daily_undelegate_limit = new_limit;
   }
