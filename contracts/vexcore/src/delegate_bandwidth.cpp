...
namespace eosiosystem {
  ...
  /**
                                       ...
                                       const asset& unstake_net_quantity, const asset& unstake_cpu_quantity )
   {
      ...
      // Vexanium: daily undelegate limit check
      if( _vexlimits.exists() ) {
         auto total_unstake = unstake_net_quantity + unstake_cpu_quantity;
         auto now_day_us = eosio::microseconds( (eosio::current_time_point().sec_since_epoch() / 86400) * 86400 * 1000000LL );
         auto now_day    = time_point( now_day_us );
         if( _gvexlimits.last_reset_time < now_day ) {
            _gvexlimits.total_undelegated_today = asset( 0, core_symbol() );
            _gvexlimits.total_withdrawn_today   = asset( 0, core_symbol() );
            _gvexlimits.last_reset_time         = now_day;
         }
         _gvexlimits.total_undelegated_today += total_unstake;
         check( _gvexlimits.total_undelegated_today <= _gvexlimits.daily_undelegate_limit,
                "daily undelegate limit exceeded" );
      }
